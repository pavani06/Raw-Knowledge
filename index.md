...
- [[concepts/agent-evals|Agent Evals]] — Three-tier evaluation framework (code → built-in → LLM-as-judge) for agentic systems, now including long-session context degradation tests (5 sources)
...
- [[concepts/agent-state-management|Agent State Management]] — Non-volatile execution and business state management enabling agent pause/resume via DB serialization (1 source)
...
- [[concepts/continuous-evaluation|Continuous Evaluation]] — Running evals on every change and on live production traffic; deployment is the start, not the end (2 sources)
...
- [[concepts/deterministic-checks|Deterministic Checks]] — Hard, code-checkable eval rules (schema, citations, leakage); the first and cheapest eval layer (2 sources)
...
- [[concepts/eval-driven-development|Eval-Driven Development]] — Writing capability evals before building features; the quality operating system for agentic AI (3 sources)
...
- [[concepts/eval-governance|Eval Governance]] — Evals as audit/compliance evidence (EU AI Act, NIST, SR 11-7); mathematical proof of safety (1 source)
...
- [[concepts/golden-dataset|Golden Dataset]] — Curated ground truth of human-labeled examples; the encoded judgment of domain experts (3 sources)
...
- [[concepts/human-in-loop-vs-afk|Human-in-Loop vs. AFK]] — Task classification: planning requires human presence; implementation can be delegated (2 sources)
...
- [[concepts/llm-as-judge|LLM-as-Judge]] — Using a separate LLM to grade semantic quality; requires CoT, single-dimension rubrics, meta-evaluation (3 sources)
...
- [[concepts/release-gates|Release Gates]] — The ship/block decision encoded as a predefined probabilistic threshold; eval back-pressure on releases (2 sources)
...
- [[concepts/sub-agents|Sub-Agents]] — Delegated agents, each with its own context window; for controlling heavy data and main-chat context (7 sources)
