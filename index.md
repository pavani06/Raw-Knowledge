...
- [[concepts/capability-escalation-ladder|Capability Escalation Ladder]] — Ordered capability levers for a failing task (bigger model → reasoning budget → better prompt → decomposition), measured per rung; the economic winner among passing routes (1 source)
...
- [[concepts/deterministic-checks|Deterministic Checks]] — Hard, code-checkable eval rules (schema, citations, leakage, regex, classic ML for NER/intent classification); the first and cheapest eval layer; hard/soft grader split with violation counts (4 sources)
...
- [[concepts/generator-evaluator-pattern|Generator-Evaluator Pattern]] — GAN-style adversarial builder/critic split; independent repairer variant and its economics (3 sources)
...
- [[concepts/model-change-management|Model Change Management]] — Testing model provider upgrades against enterprise-specific eval datasets; public benchmarks are not useful in your enterprise context; migration diagnostic + patch audit (2 sources)
...
- [[concepts/prompt-as-code|Prompt-as-Code]] — Prompt versioning treated as change management: commit messages documenting what failure caused the change and what it corrects; full traceability required; Google-Doc-managed agent instructions as the cautionary tale; defensive patch ledger (4 sources)
...
- [[concepts/structured-output|Structured Output]] — Factor 1 of 12-Factor Agents: the most magical LLM capability is turning natural language into structured JSON; two-layer output contract (3 sources)
...
- [[concepts/tool-use|Tool Use]] — Agents calling search, files, APIs, and other external capabilities to act on the world; includes tool definition and sequential calling patterns; instructions don't add capability (5 sources)
...
- [[concepts/two-sided-trade-off-instruction|Two-Sided Trade-off Instruction]] — State the cost AND the counter-cost of actions the prompt controls; single-objective overfit; withholding as inverse hallucination; ban lists become source-of-truth designations (1 source)
...
- [[sources/2026-09-02-the-prompting-playbook|The Prompting Playbook]] — Claude (Anthropic, Code with Claude breakout) on the prompting playbook: eval-gated model migration, prompt hygiene, two-layer output contracts, tool integration, trade-off balancing, and the capability escalation ladder (video, 2026-09-02)
