...
type: source
source_type: video
source: "https://youtu.be/am_oeAoUhew?si=0I0ta4wvDAF27uSu"
...
tags: [ai, agents, llm, software-engineering, coding-agents, harness-engineering]
concepts:
  - "[[concepts/harness-engineering]]"
  - "[[concepts/non-functional-requirements]]"
  - "[[concepts/code-as-free-resource]]"
  - "[[concepts/reviewer-agents]]"
  - "[[concepts/codebase-uniformity]]"
  - "[[concepts/garbage-collection-day]]"
  - "[[concepts/prompt-injection-patterns]]"
...
[music] execute. Please join me in
welcoming to the stage member of
technical staff at OpenAI, Ryan Leopo.
[applause]
[cheering]
Good morning, London.
[applause]
...
your actual working setup with
[laughter] no laptop? Um,
>> uh, yeah. Uh, here. Beach [laughter]
Margarita
linear, right?
>> Oh, wow.
...
completion.
>> Wow, good statement at the end.
[laughter] every time you have to
interact with the agent is a failure.
Okay, so the following question kind of
scales this out, right? As your org
knowledge map scales, what practical
steps do you have to like enable
progressive disclosure? So as you have a
larger and larger codebase, as you have
more people, how do you scale your
agents to work better with this?
>> Yeah. So
...
- **Implementation is no longer the scarce resource** — code is free, infinitely parallel, and disposable. The scarce resources are human time, human/model attention, and context window ([[concepts/code-as-free-resource]]).
- **Harness engineering** is the discipline of building structures (docs, guardrails, lint rules, review agents) that let agents do the full job without continuous human steering ([[concepts/harness-engineering]]).
- **Non-functional requirements must be written down** — agents have seen trillions of lines of code and can satisfy any style, but only if you specify it. Undocumented taste is invisible to the model ([[concepts/non-functional-requirements]]).
- **Just-in-time context surfacing** beats front-loading: defer instructions to lint/test time so the agent can prototype freely, then receive targeted corrections ([[concepts/context-window-management]]).
- **Reviewer agents replace synchronous human code review** — persona-based agents (front-end architect, reliability engineer, etc.) trigger on every push and surface P2+ issues automatically ([[concepts/reviewer-agents]]).
- **Codebase uniformity is a force multiplier** — one way to do everything (one ORM, one CI script pattern, one async helper) means the model's token predictions are maximally transferable across the repo ([[concepts/codebase-uniformity]]).
- **Garbage Collection Day** — dedicate one day per week to categorically eliminating slop patterns observed during the week, converting human review feedback into durable automated guardrails ([[concepts/garbage-collection-day]]).
- **Every human interaction with the agent is a harness failure** — if you have to type "continue," the harness didn't provide enough context to reach completion autonomously ([[concepts/agent-harness]]).
- **Skills: go deep, not wide** — maintain 5–10 high-quality skills rather than sprawling many thin ones; hide infrastructure complexity beneath skills so agents don't need to track it ([[concepts/skills-progressive-disclosure]]).
- **Code as compiled artifact** — the LLM is a fuzzy compiler; harness constraints are optimization passes; swapping models is like changing a code-generation backend ([[concepts/code-as-free-resource]]).

## Concepts

- [[concepts/harness-engineering|Harness Engineering]] — the overarching philosophy: humans steer (define work, write guardrails), agents execute
- [[concepts/non-functional-requirements|Non-Functional Requirements]] — the undocumented taste/quality bar that must be written down for agents
- [[concepts/code-as-free-resource|Code as Free Resource]] — the axiom that code is abundant, disposable, and no longer the scarce resource
- [[concepts/reviewer-agents|Reviewer Agents]] — automated agents that inject persona-based feedback into PRs on every push
- [[concepts/codebase-uniformity|Codebase Uniformity]] — making code maximally consistent to reduce agent attention cost
- [[concepts/garbage-collection-day|Garbage Collection Day]] — weekly ritual to convert human review feedback into automated guardrails
- [[concepts/prompt-injection-patterns|Prompt Injection Patterns]] — the taxonomy of ways to surface context to agents (lint errors, review agents, skills, AGENTS.md, test failures)
- [[concepts/agent-harness|Agent Harness]] — compounded: Ryan's framing as "giving the model text at the right time"
- [[concepts/skills-progressive-disclosure|Skills (Progressive Disclosure)]] — compounded: 5–10 deep skills > many shallow ones
- [[concepts/context-window-management|Context Window Management]] — compounded: just-in-time surfacing pattern
- [[concepts/sub-agents|Sub-Agents]] — compounded: reviewer agents as sub-agents triggered on push
- [[concepts/verification-loop|Verification Loop]] — compounded: QA plans + review agents as structured verification

## Entities

- [[entities/openai|OpenAI]] — Ryan's employer; context for the scale of agent usage described
- [[entities/codex|Codex]] — OpenAI's coding agent (the primary harness Ryan's team uses)
- [[entities/linear|Linear]] — ticket/project management tool used as the entry point for agent work
- [[entities/claude-code|Claude Code]] — mentioned as a comparable harness alongside Codex

## Connections

- Extends [[concepts/agent-harness]]: Ryan's "harness engineering" is a named methodology built on top of the general harness concept — adds the human-steering / agent-executing split and the operational practices around it
- Contradicts the "more skills = better" intuition: Ryan explicitly argues for fewer, deeper skills ([[concepts/skills-progressive-disclosure]])
- Supports [[concepts/context-window-management]]: just-in-time context surfacing is a concrete implementation of context management discipline
- Relates to [[concepts/verification-loop]]: QA plans + review agents are a structured, automated form of verification
- Supports [[concepts/code-as-free-resource]]: the "LLM as fuzzy compiler" mental model reframes code as a compiled artifact, not a precious hand-crafted asset
