...
- [[concepts/agent-evals|Agent Evals]] — Three-tier evaluation framework (deterministic → semantic/LLM-as-judge → behavioral) for agentic systems; now spans code-level checks, long-session context degradation tests, tool-call correctness, duplicate API call detection, and quality-over-coverage scoping (8 sources)
...
- [[concepts/agent-tracing|Agent Tracing]] — Centralized cross-framework trace collection strategy; serving traces to multiple downstream consumers: dashboards, auditors, LLM judges, online monitors (1 source)
...
- [[concepts/continuous-evaluation|Continuous Evaluation]] — Running evals on every change and on live production traffic; automated pipeline capturing responses, comparing against eval dataset, routing below-threshold to human review (3 sources)
...
- [[concepts/data-flywheel|Data Flywheel]] — Compounding eval data as a competitive moat; each iteration deepens the dataset; test case library as a living system that grows with every production incident; demand-side flywheel from classified question logs (4 sources)
- [[concepts/data-foundation|Data Foundation]] — Question data (serving AI) vs tracking data (observability data); centralized trace collection across heterogeneous agent frameworks (1 source)
...
- [[concepts/eval-driven-development|Eval-Driven Development]] — Writing capability evals before building features; the quality operating system for agentic AI; model selection deferred until after eval infrastructure built (4 sources)
...
- [[concepts/golden-dataset|Golden Dataset]] — Curated ground truth of human-labeled examples; the encoded judgment of domain experts; a living system that starts small (~200 cases) and grows forever as production incidents are captured (4 sources)
...
- [[concepts/llm-as-judge|LLM-as-Judge]] — Using a separate LLM to grade semantic quality (safety, groundedness, relevance); requires CoT, single-dimension rubrics, meta-evaluation; runs automatically on traces (4 sources)
...
- [[entities/langchain|LangChain]] — Agent framework; enterprises often run agents across multiple frameworks (CrewAI, LangChain, etc.) simultaneously (1 source)
...
- [[entities/snowflake|Snowflake]] — Data cloud company (~10k employees, half GTM); customer zero for its own agent platform (1 source)
