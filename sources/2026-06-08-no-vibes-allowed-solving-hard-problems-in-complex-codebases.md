...
type: source
source_type: video
source: "https://youtu.be/rmvDxxNubIg?si=SBp9cGOq9rpNXHP2"
...
tags: [ai, agents, llm, coding-agents, context, workflow, software-engineering]
concepts: ["[[concepts/research-plan-implement]]", "[[concepts/compaction]]", "[[concepts/smart-zone-dumb-zone]]", "[[concepts/context-window-management]]", "[[concepts/sub-agents]]", "[[concepts/harness-engineering]]", "[[concepts/mental-alignment]]", "[[concepts/brownfield-codebases]]", "[[concepts/dont-outsource-thinking]]", "[[concepts/semantic-diffusion]]", "[[concepts/vertical-slices]]"]
entities: ["[[entities/humanlayer]]"]
status: processed
---
...
> Source: https://youtu.be/rmvDxxNubIg?si=SBp9cGOq9rpNXHP2
> Channel: AI Engineer · Extracted: 2026-06-08 · Method: api
...
- Naive AI coding produces rework and slop in [[concepts/brownfield-codebases|brownfield codebases]]; disciplined [[concepts/context-window-management|context engineering]] is what makes AI viable on complex/legacy code ("no slop").
- [[concepts/research-plan-implement|Research → Plan → Implement (RPI)]] is the core workflow: each phase compacts the prior phase's output so the agent stays in the [[concepts/smart-zone-dumb-zone|smart zone]].
- [[concepts/compaction|Intentional compaction]] = compress the context window into a reviewable, taggable markdown file; "frequent intentional compaction" builds the whole workflow around keeping context small.
- The [[concepts/smart-zone-dumb-zone|dumb zone]] starts around the ~40% line of the window; too many MCPs dumping JSON push you into it before you start. LLMs are stateless, not pure functions — better tokens in, better tokens out.
- [[concepts/sub-agents|Sub-agents are for controlling context, not anthropomorphizing roles]] — fork out, do the reading, return a succinct pointer to the parent.
- Code review is for [[concepts/mental-alignment|mental alignment]]: review the plans, not every line of code, to keep the team aligned as throughput scales.
- [[concepts/dont-outsource-thinking|Don't outsource the thinking]] — AI amplifies (or exposes the absence of) human thinking; there is no perfect prompt and no silver bullet.
- [[concepts/semantic-diffusion|Semantic diffusion]] (Fowler, 2006) killed "spec-driven dev" and "agent" as useful terms; Dex resists the RPI acronym for the same reason.
- The coding-agent layer will be commoditized; the durable problem is adapting team/SDLC to a world where 99% of code ships by AI. Pick one tool and get reps.

## Concepts

- [[concepts/research-plan-implement|Research-Plan-Implement (RPI)]] — the talk's central workflow; three context-compaction phases (NEW)
- [[concepts/compaction|Compaction]] — intentional compaction to a reviewable markdown file; what-to-compact; compaction = avoiding the dumb zone (compounded 2→3)
- [[concepts/smart-zone-dumb-zone|Smart Zone / Dumb Zone]] — canonical origin source; ~40% line; Jeff Huntley's law; stateless-not-pure (compounded 1→2, fixed Horthy attribution)
- [[concepts/context-window-management|Context Window Management]] — "context engineering" as the named umbrella; four optimization axes + trajectory; compressing truth (compounded 3→4)
- [[concepts/sub-agents|Sub-Agents]] — "context not roles"; fork-out-and-return-succinct (compounded 3→4)
- [[concepts/harness-engineering|Harness Engineering]] — framed as "part of context engineering"; commoditization + seniority rift (compounded 2→3)
- [[concepts/mental-alignment|Mental Alignment]] — code review's true purpose; reviewing plans not code (NEW)
- [[concepts/brownfield-codebases|Brownfield Codebases]] — the greenfield/brownfield split; the problem domain RPI targets (NEW)
- [[concepts/dont-outsource-thinking|Don't Outsource the Thinking]] — AI amplifies thinking; no perfect prompt; markdown theater (NEW)
- [[concepts/semantic-diffusion|Semantic Diffusion]] — why "spec-driven dev" and "agent" became useless; resisting RPI (NEW)
- [[concepts/vertical-slices|Vertical Slices]] — vertical slices through the codebase as a research primitive (compounded 1→2)

## Entities

- [[entities/humanlayer|HumanLayer]] — Dex Horthy's company; originator of RPI and the viral open-source research/plan/implement prompts; building an agentic IDE (NEW)

## Connections

- Compounds the entire **agent-harness cluster** ([[concepts/_moc-agent-harness]]) from the context-engineering angle — this is the HumanLayer counterpart to Ryan Lopopolo's [[concepts/harness-engineering]] and Anthropic's [[concepts/planner-generator-evaluator-architecture]].
- Resolves the prior misattribution of [[concepts/smart-zone-dumb-zone|the dumb zone]] to "Dex Hardy" — this is Dex Horthy's own talk, the canonical origin Matt Pocock was relaying.
- New tension flagged: [[concepts/harness-engineering]] (Ryan: harness engineering is the top methodology) vs. this source (Dex: harness engineering is "part of context engineering"). Same elephant, opposite ends.
- New tension flagged: dumb-zone threshold at ~100K absolute (Matt) vs. ~40% of window (Dex) — both rough guidelines for the same degradation.
- [[concepts/mental-alignment]] complements [[concepts/reviewer-agents]]: machines enforce correctness, humans read plans to stay aligned.
