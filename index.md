...
- [[concepts/agent-evals|Agent Evals]] — Three-tier evaluation framework (deterministic → semantic/LLM-as-judge → behavioral) for agentic systems; now spans code-level checks, long-session context degradation tests, tool-call correctness, and duplicate API call detection (8 sources)
...
- [[concepts/data-flywheel|Data Flywheel]] — Compounding eval data as a competitive moat; each iteration deepens the dataset; test case library as a living system that grows with every production incident (3 sources)
...
- [[concepts/feedback-loops|Feedback Loops]] — Tests, types, and linters as the quality ceiling for AI coding output; CSAT drop → trace investigation → fix → test-case feedback loop in production AI (2 sources)
...
- [[concepts/prompt-as-code|Prompt-as-Code]] — Prompt versioning treated as change management: commit messages documenting what failure caused the change and what it corrects; full traceability required (1 source)
...
- [[concepts/release-gates|Release Gates]] — The ship/block decision encoded as a predefined probabilistic threshold; eval back-pressure on releases; cost governance via subset eval in CI, full test on merge (3 sources)
